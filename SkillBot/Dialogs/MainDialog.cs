...
namespace Microsoft.BotBuilderSamples.SkillBot.Dialogs
{
    public class MainDialog : ComponentDialog
    {
            ...
            : base(nameof(MainDialog))
        {
            ...
            AddDialog(new TestActionDialog(configuration));
        }
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (innerDc.Context.Activity.Type == ActivityTypes.Message)
            {
                ...
                else if (text == "testaction" || text == "test action" || text == "a1")
                {
                    dialogResult = await innerDc.BeginDialogAsync(nameof(TestActionDialog), null, cancellationToken);
                }
                else
                {
